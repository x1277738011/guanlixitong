...
public class coursequery extends PublicJFrame {
    /** 成员变量： */
    ...
    // 调用方法，获取所有图书列表，并保存
    private List<course> list = Courses.selectcoursesList();
    // 创建图书表栏目名称字符串
    private String[] tb_heads = { "课程号 ", "课程名 ", "开设学院", "学时", "学分",};
    // 图书表栏目名称对应的表字段
    private String[] fields = { "courseno", "coursename", "courses",
            "coursetime","credits" };
    // 创建图书表格模型
    private DefaultTableModel model = new DefaultTableModel(
            new Object[][] {}, tb_heads);
    private String field = "courseno";// 设置默认的当前的查询字段
    ...
    /** 成员方法3：向表格模型，添加图书列表 */
    private void addRowToModel(List<course> list) {
        model.setRowCount(0);// 清除表格模型中的数据
        for (int i = 0; i < list.size(); i++) {
            course cours = list.get(i);
            model.addRow(new Object[] { cours.getCourseno(), cours.getCoursenoname(),
                    cours.getCourses(), cours.getCoursetime(),cours.getCredits(),});
        }
    }
    ...
    private void cb_query_itemStateChanged(ItemEvent arg0) {
        if (arg0.getStateChange() == ItemEvent.SELECTED) {
            ...
            list = Courses.selectcoursesList();// 调用方法，获取所有图书列表
    ...
    private void bn_query_actionPerformed() {
        ...
        switch (field) {
            ...
            case "courses":
                ...
                list = Courses.selectcoursesList(field,valueStr);
                break;
            case "coursetime":
            case "credits":
                ...
                list = Courses.selectcoursesList(field, valueInt);
                break;
        }
