...
public class chosecourse extends PublicJFrame {
    ...
    chosecourse() {
        ...
        innerPane.setBorder(new TitledBorder(
                ...
                "选课", TitledBorder.LEADING, TitledBorder.TOP,
    ...
    public void refresh() {
        ...
        list = clas.selectClasssList();// 获取对象列表
    ...
    public void jb_insert_actionPerformed() {
        ...
        if (tf_name.getText().trim().equals("")
            ...
            JOptionPane.showMessageDialog(null, "选课成功！");// 执行添加操作
            if (i == 1) {// 判断添加操作是否成功？若成功，则向表格模型添加这条数据
                model.addRow(new Object[] {  });
                refresh();
            }
            else { JOptionPane.showMessageDialog(null, "选课失败！");  }
            del_content();
        }
    }
    ...
    public void jb_delete_actionPerformed() {
        ...
        if (m == JOptionPane.YES_OPTION) {// 如果确定，则执行删除操作
            if (chose.deletescores(no,id) == 1) {// 执行用户删除操作
                refresh();// 更新表格内容
