...
public class scorequery extends PublicJFrame {
    /** 成员变量： */
    ...
    // 调用方法，获取所有图书列表，并保存
    private List<score> list = Score.selectscoresList();
    // 创建图书表栏目名称字符串
    private String[] tb_heads = { "学号 ", "姓名 ", "课程号", "课程名", "分数",
            "班级号","补考分数"};
    // 图书表栏目名称对应的表字段
    private String[] fields = { "studentno", "studentname", "courseno",
            "coursename","score" ,"classno","fillscore"};
    // 创建图书表格模型
    private DefaultTableModel model = new DefaultTableModel(
            new Object[][] {}, tb_heads);
    private String field = "studentno";// 设置默认的当前的查询字段
    ...
    /** 成员方法3：向表格模型，添加列表 */
    private void addRowToModel(List<score> list) {
        model.setRowCount(0);// 清除表格模型中的数据
        for (int i = 0; i < list.size(); i++) {
            score scor = list.get(i);
            model.addRow(new Object[] { scor.getStudentno(), scor.getStudentname(),
                    scor.getCourseno(), scor.getCoursename(),scor.getScore(),
                    scor.getTearchername(),scor.getFillscore()});
        }
    }
    ...
    private void cb_query_itemStateChanged(ItemEvent arg0) {
        if (arg0.getStateChange() == ItemEvent.SELECTED) {
            ...
            list = Score.selectscoresList();// 调用方法，获取所有图书列表
    ...
    private void bn_query_actionPerformed() {
        ...
        switch (field) {
            ...
            case "classno":
                ...
                list = Score.selectscoresList(field,valueStr);
                break;
            case "studentno":
            case "score":
            case "fillscore":
                ...
                list = Score.selectscoresList(field, valueInt);
                break;
        }
