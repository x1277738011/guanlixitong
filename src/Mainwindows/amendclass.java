...
public class amendclass extends PublicJFrame {
    /** 成员变量 */
    private JTextField tf_id;// 声明图书编号文本框
    private JTextField tf_name;// 声明书名文本框
    private JTextField tf_courseanme;// 声明译者文本框
    private JTextField tf_tearchno;// 声明出版社文本框
    private JTextField tf_tearchname;
    private JTextField tf_students;
    ...
    private DefaultTableModel model = new DefaultTableModel(
            new Object[][] {}, new String[] { "班级号", "课程号", "课程名",
            "教师号", "教师","学生人数" });
    private List<Class> list = clas.selectClasssList();// 创建所有图书列表，并保存图书对象

    /** 构造方法：用于初始化界面 */
    amendclass() {
        // 界面设置
        this.setTitle("--班级信息编辑--");
    ...
    private JPanel createDataPanel() {
        ...
        JLabel jl_type = new JLabel("课程名");
        jl_type.setBounds(50, 100, 150, 25);
        dataPanel.add(jl_type);

        tf_courseanme= new JTextField(10);
        tf_courseanme.setBounds(140, 100, 170, 25);
        dataPanel.add(tf_courseanme);

        JLabel jl_sex = new JLabel("教师号");
        jl_sex.setBounds(50, 140, 150, 25);
        dataPanel.add(jl_sex);

        tf_tearchno = new JTextField(10);
        tf_tearchno.setBounds(140, 140, 170, 25);
        dataPanel.add(tf_tearchno);

        JLabel jl_translator = new JLabel("教师:");
        jl_translator.setBounds(50, 180, 150, 25);
        dataPanel.add(jl_translator);

        tf_tearchname = new JTextField(10);
        tf_tearchname.setBounds(140, 180, 170, 25);
        dataPanel.add(tf_tearchname);

        JLabel jl_student = new JLabel("学生人数");
        jl_student.setBounds(50, 220, 150, 25);
        dataPanel.add(jl_student);

        tf_students = new JTextField(10);
        tf_students.setBounds(140, 220, 170, 25);
        dataPanel.add(tf_students);
    ...
    private JPanel createTablePanel() {
        ...
        for (int i = 0; i < list.size(); i++) {
            Class courses = list.get(i);
            model.addRow(new Object[] { courses.getClassno(), courses.getCourseno(),
                    courses.getCoursename(), courses.getTearcherno(),courses.getTearchername(),
                    courses.getStudents()});
        }
    ...
    public void del_content() {
        tf_id.setText("");
        tf_name.setText("");
        tf_courseanme.setText("");
        tf_tearchno.setText("");
        tf_tearchname.setText("");
        tf_students.setText("");
    ...
    public void tf_id_focusLost() {
        // 通过图书编号，获取图书对象
        Class course = clas.getClasssById(tf_id.getText().trim());
    ...
    public void refresh() {
        ...
        list = clas.selectClasssList();// 获取图书对象列表
    ...
    public void table_mouseClicked() {
        // 获取选中信息
        Class course_old = list.get(table.getSelectedRow());
        // 在数据面板中设置对应的图书数据
        tf_id.setText(course_old.getClassno());
        tf_name.setText(course_old.getCourseno());
        tf_courseanme.setText(course_old.getCoursename());
        tf_tearchno.setText(course_old.getTearcherno());
        tf_tearchname.setText(course_old.getTearchername());
        tf_students.setText(String.valueOf(course_old.getStudents()));
        tf_id.setEditable(false);// 设置课程号文本框不可编辑
    }

    /** 成员方法9：添加方法 */
    public void jb_insert_actionPerformed() {
        Class courses = new Class();// 创建一个对象

        courses.setClassno(tf_id.getText().trim());
        courses.setCourseno(tf_name.getText().trim());
        courses.setCoursename(tf_courseanme.getText().trim());
        courses.setTearcherno(tf_tearchno.getText().trim());
        courses.setTearchername(tf_tearchname.getText().trim());
        courses.setStudents(new Integer(tf_students.getText().trim()));
        ...
        if (tf_name.getText().trim().equals("")
            ...
            int i = clas.insertClasss(courses);// 执行添加操作
    ...
    public void jb_update_actionPerformed() {
        ...
        if (tf_name.getText().trim().equals("")
            ...
            int i = clas.updateClasss(courses);
            if (i == 1) {
    ...
    public void jb_delete_actionPerformed() {
        ...
        if (m == JOptionPane.YES_OPTION) {// 如果确定，则执行删除操作
            if (clas.deleteClasss(id) == 1) {// 执行用户删除操作
                refresh();// 更新表格内容
