...
public class amendscore extends PublicJFrame {
    /** 成员变量 */
    ...
    private DefaultTableModel model = new DefaultTableModel(
            ...
            "课程名", "分数", "班级号","补考分数" });
    private List<score> list = Score.selectscoresList();// 创建所有图书列表，并保存图书对象
    ...
    private JPanel createDataPanel() {
        ...
        JLabel jl_publisher = new JLabel("班级号:");
    ...
    public void tf_id_focusLost() {
        // 通过图书编号，获取图书对象
        score scores = Score.getscoresById(new Integer(tf_id.getText().trim()));
    ...
    /** 成员方法6：文本框失去焦点事件响应具体方法 */
    public void tf_name_focusLost() {
        score scores = Score.getscoresByName(tf_no.getText().trim());
        if (scores != null) {
    ...
    public void refresh() {
        ...
        list = Score.selectscoresList();// 获取对象列表
    ...
    public void table_mouseClicked() {
        ...
        tf_id.setEditable(false);// 设置文本框不可编辑
    }
    ...
    public void jb_insert_actionPerformed() {
        ...
        if (tf_name.getText().trim().equals("")
            ...
            int i = Score.insertscores(scores);// 执行添加操作
    ...
    public void jb_update_actionPerformed() {
        ...
        if (tf_name.getText().trim().equals("")
            ...
            int i = Score.updatescores(scores);
            if (i == 1) {
    ...
    public void jb_delete_actionPerformed() {
        ...
        if (m == JOptionPane.YES_OPTION) {// 如果确定，则执行删除操作
            if (Score.deletescores(no,id) == 1) {// 执行用户删除操作
                refresh();// 更新表格内容
