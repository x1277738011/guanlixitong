...
public class addstudent extends PublicJFrame {
    /** 成员变量 */
    ...
    private JTextField tf_getcredits;// 声明出版社文本框
    ...
    private DefaultTableModel model = new DefaultTableModel(
            new Object[][] {}, new String[] { "学号", "学生姓名", "性别",
            "年龄", "所属院系", "获得学分" });
    private List<students> list = Students.selectstudentList();// 创建所有图书列表，并保存图书对象

    /** 构造方法：用于初始化界面 */
    addstudent() {
        // 界面设置
        this.setTitle("--学生信息编辑--");
        ...
        innerPane.setBorder(new TitledBorder(
                ...
                "学生信息", TitledBorder.LEADING, TitledBorder.TOP,
    ...
    private JPanel createDataPanel() {
        ...
        JLabel jl_author = new JLabel("性别：");
        ...
        JLabel jl_publisher = new JLabel("获得学分:");
        jl_publisher.setBounds(50, 220, 150, 25);
        dataPanel.add(jl_publisher);

        tf_getcredits = new JTextField(10);
        tf_getcredits.setBounds(140, 220, 170, 25);
        dataPanel.add(tf_getcredits);
    ...
    private JPanel createTablePanel() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            model.addRow(new Object[] { student.getStudentno(), student.getStudentnme(),
                    student.getDepartmentname(), student.getSex(),
                    student.getAge(), student.getGetcredits(),});
        }
    ...
    public void tf_id_focusLost() {
        // 通过图书编号，获取图书对象
        students student = Students.getstudentById(Integer.parseInt(tf_id.getText().trim()));
        if (student != null) {// 判断图书对象是否存在，若存在，清除数据，重新输入读者编号
    ...
    public void tf_name_focusLost() {
        students student = Students.getstudentByName(tf_name.getText().trim());
        if (student != null) {
    ...
    public void refresh() {
        ...
        list = Students.selectstudentList();// 获取图书对象列表
    ...
    public void table_mouseClicked() {
        // 获取选中图书的信息
        students student_old = list.get(table.getSelectedRow());
        // 在数据面板中设置对应的图书数据
        tf_id.setText(String.valueOf(student_old.getStudentno()));
        tf_name.setText(student_old.getStudentnme());
        tf_sex.setText(student_old.getSex());
        tf_age.setText(String.valueOf(student_old.getAge()));
        cb_type.setSelectedItem(student_old.getDepartmentname());
        tf_getcredits.setText(String.valueOf(student_old.getGetcredits()));
        tf_id.setEditable(false);// 设置读者图书文本框不可编辑
    }

    /** 成员方法9：添加新图书方法 */
    public void jb_insert_actionPerformed() {
        students student = new students();// 创建一个图书对象
        // 设置图书对象的属性值
        student.setStudentno(Integer.parseInt(tf_id.getText().trim()));
        student.setStudentnme(tf_name.getText().trim());
        student.setSex(tf_sex.getText().trim());
        student.setAge(new Integer(tf_age.getText().trim()));
        student.setDepartmentname(cb_type.getSelectedItem().toString());
        student.setGetcredits(new Integer(tf_getcredits.getText().trim()));
        ...
        if (tf_name.getText().trim().equals("")
            ...
            int i = Students.insertstudent(student);// 执行添加操作
    ...
    public void jb_update_actionPerformed() {
        students student = new students();// 创建一个图书对象
        student.setStudentno(new Integer(tf_id.getText().trim()));
        student.setStudentnme(tf_name.getText().trim());
        student.setSex(tf_sex.getText().trim());
        student.setAge(new Integer(tf_age.getText().trim()));
        student.setDepartmentname(cb_type.getSelectedItem().toString());
        ...
        if (tf_name.getText().trim().equals("")
            ...
            int i = Students.updatestudent(student);
            if (i == 1) {
    ...
    public void jb_delete_actionPerformed() {
        ...
        if (m == JOptionPane.YES_OPTION) {// 如果确定，则执行删除操作
            if (Students.deletestudent(id) == 1) {// 执行用户删除操作
                refresh();// 更新表格内容
