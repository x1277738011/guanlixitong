...
public class StudentsQuery extends PublicJFrame {
    /** 成员变量： */
    ...
    // 调用方法，获取所有图书列表，并保存
    private List<students> list = Students.selectstudentList();
    // 创建图书表栏目名称字符串
    private String[] tb_heads = { "学号 ", "学生姓名 ", "所属院系", "性别", "年龄",
            "累计学分"};
    // 图书表栏目名称对应的表字段
    private String[] fields = { "studentno", "studentname", "sex", "age",
            "departmentname", "getcredits"};
    // 创建图书表格模型
    ...
    /** 成员方法3：向表格模型，添加图书列表 */
    private void addRowToModel(List<students> list) {
    ...
    private void cb_query_itemStateChanged(ItemEvent arg0) {
        if (arg0.getStateChange() == ItemEvent.SELECTED) {
            ...
            list = Students.selectstudentList();// 调用方法，获取所有图书列表
    ...
    private void bn_query_actionPerformed() {
        ...
        switch (field) {
            ...
            case "departmentname":
                ...
                list = Students.selectstudentList(field,valueStr);
                break;
            case "getcredits":
            case "age":
            case "studentno":
                ...
                list = Students.selectstudentList(field, valueInt);
                break;
        }
